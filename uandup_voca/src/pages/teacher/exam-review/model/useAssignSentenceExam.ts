...
// 채점 화면에서 곧바로 다음 단계(Sentence/EXAMPLE) 시험을 배정+응시개시하는 mutation.
//
...
export function useAssignSentenceExam({ studentId, studySetId }: Params) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async () => {
      const res = await createExam(studySetId, { examType: 'EXAMPLE' });
    ...
    // 실패한 부분 실패 상황에서도 시험은 이미 생성됐으므로, 상태를 갱신해야 배정 버튼이
    // (Sentence step이 pending→active로 바뀌며) 사라진다. onSuccess만 걸면 그 경우 버튼이
    // 남아 재클릭 시 EXAM_ALREADY_ACTIVE 에러로 이어진다.
    onSettled: () => {
      // studySets를 갱신해야 이 페이지의 "배정 가능" 판정(Sentence step 상태)이 즉시 닫힌다.
      queryClient.invalidateQueries({ queryKey: studentKeys.studySets(studentId) });
      // EXAMPLE history도 무효화 — 안 하면 클리닉 화면의 Sentence step이 낡은 상태로 보인다.
      queryClient.invalidateQueries({ queryKey: testKeys.history(studySetId, 'EXAMPLE') });
    },
