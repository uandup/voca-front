...
interface PersonalWordFlashcardProps {
  words: PersonalWordCardData[];
...
// widgets/word-flashcard의 WordFlashcard와 같은 네비게이션/뒤집기 상호작용을 갖지만,
// PersonalWord는 word/koreanMeaning만 있어 WordCardData(품사·영영뜻·동의어 등)에 억지로
// 맞추지 않는다 — 뒷면에 "Meaning" 헤더나 빈 영영뜻 줄 같은 빈 자리가 남는 걸 피하기 위해
// PersonalWordListPage 전용으로 따로 둔다.
export function PersonalWordFlashcard({
  words,
  bookmarkedIds,
  onToggleBookmark,
}: PersonalWordFlashcardProps) {
  ...
  function MeaningPanel({ isBack }: { isBack: boolean }) {
    return (
      <div
        className="absolute inset-0 bg-white border border-outline-variant/40 rounded-2xl shadow-md flex flex-col items-center justify-center gap-5 px-12 select-none"
        style={faceStyle(isBack)}
      >
        <p className="text-primary font-bold text-4xl leading-snug text-center">
          {word.koreanMeaning}
        </p>
      </div>
    );
